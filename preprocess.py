...
class Preprocess:
    ...
    def get_text_contour(self, contours):
        # loop over the contours
        for c in contours:
            approx = self.approximate_contour(c)
            # if our approximated contour has four points, we can assume it is text's rectangle
            if len(approx) == 4:
                return approx

    def image_with_text_contour(self, largest_contours, img_largest_contours):
        text_contour = self.get_text_contour(largest_contours)
        image_with_text_contour = cv2.drawContours(img_largest_contours.copy(), [text_contour], -1, (0, 255, 0),
                                                      2)
        # cv2.imshow("Text Contour Image ", image_with_text_contour)
        # cv2.waitKey(0)
        # cv2.destroyAllWindows()
        return text_contour, image_with_text_contour
    ...
    def warp_perspective(self, img, text_contour):
        # unpack rectangle points: top left, top right, bottom right, bottom left
        rect = self.contour_to_rect(contour=text_contour)
        (tl, tr, br, bl) = rect
...
if __name__ == '__main__':
    path = "F:/Projects/Handwritten Digits Recognition/Test_images/3.png"
